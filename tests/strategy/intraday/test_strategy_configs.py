...
class TestInfraExitConfig:
    ...
    def test_defaults(self) -> None:
        ...
        assert cfg.be_trigger == _D("0.5")
        assert cfg.trail_buf == _D("0.15")
        assert cfg.orb_trail_atr == _D("1.5")
...
class TestPullbackStrategyConfig:
    ...
    def test_pb_defaults(self) -> None:
        ...
        assert cfg.pb_body == _D("0.15")
        assert cfg.rr == _D("1.4")
        assert cfg.sl_atr == _D("1.3")
        assert cfg.sl_cap == _D("1.50")
        assert cfg.trend_bars == 3
        assert cfg.adx_thresh == _D("21")
        assert cfg.max_day == 2
        assert cfg.spacing == 3
        assert cfg.pb_tp_mode == "rr"
        assert cfg.pb_vwap_bias is False
...
class TestReversalStrategyConfig:
    ...
    def test_rev_defaults(self) -> None:
        ...
        assert cfg.rev_band == "2σ"
        assert cfg.rev_body == _D("0.20")
        assert cfg.rev_sl_atr == _D("1.0")
        assert cfg.rev_shorts is False
        assert cfg.rev_min_rr == _D("0.3")
        assert cfg.rev_can_trade_bar == 0
        assert cfg.rev_min_touches == 0
        assert cfg.rev_hug_limit == 20
        assert cfg.rev_tp_mode == "vwap"
        assert cfg.rev_rr == _D("1.0")
        assert cfg.rev_require_sr is False

    def test_replace_override(self) -> None:
        cfg = dataclasses.replace(ReversalStrategyConfig(), rev_hug_limit=15)
        assert cfg.rev_hug_limit == 15
        assert cfg.rev_body == _D("0.20")  # unchanged
...
class TestORBreakoutStrategyConfig:
    ...
    def test_orb_defaults(self) -> None:
        ...
        assert cfg.orb_window == 60
        assert cfg.orb_rvol == _D("0.8")
        assert cfg.orb_sl_mode == "OR Opposite"
        assert cfg.orb_vwap_align is False
        assert cfg.orb_body_min == _D("0.10")
        assert cfg.orb_entry_mode == "aggressive"
        assert cfg.orb_tp_mode == "trail_only"
        assert cfg.orb_gap_filter is False
        assert cfg.orb_adx_filter is False
...
class TestORReversalStrategyConfig:
    ...
    def test_orr_defaults(self) -> None:
        ...
        assert cfg.orr_window == 60
        assert cfg.orr_prox == _D("0.8")
        assert cfg.orr_sl_atr == _D("0.5")
        assert cfg.orr_tp_mode == "VWAP"
        assert cfg.orr_min_rr == _D("0.3")
        assert cfg.orr_max_rr == _D("3.0")
        assert cfg.orr_rvol == _D("0.8")
        assert cfg.orr_vwap_disagree is False
        assert cfg.orr_require_break is False
...
class TestPatternScalpStrategyConfig:
    ...
    def test_ps_defaults(self) -> None:
        ...
        assert cfg.ps_window == 50
        assert cfg.ps_engulf == _D("0.15")
        assert cfg.ps_rvol == _D("0.8")
        assert cfg.ps_sma_filter is False
        assert cfg.ps_sl_mode == "Day Extreme"
        assert cfg.ps_tp_pct == _D("75.0")

    def test_ps_atr_pct_from_base(self) -> None:
        """ps_atr_pct lives in InfraExitConfig for manipulation detection."""
        cfg = PatternScalpStrategyConfig()
        assert cfg.ps_atr_pct == _D("15.0")
...
class TestConfigIndependence:
    ...
    def test_all_share_infra_defaults(self) -> None:
        """All configs share the same infra defaults."""
        configs = [
            PullbackStrategyConfig(),
            ReversalStrategyConfig(),
            ORBreakoutStrategyConfig(),
