"""Unit tests for SecOwnershipClient — EDGAR search, caching, network."""
...
from stockdownloader.data.sec.ownership_parser import find_infotable_url
...
# ------------------------------------------------------------------
# Sample EFTS / index JSON responses
# ------------------------------------------------------------------
...
class TestFetchEftsPage:
    ...
    @patch("stockdownloader.data.sec.common.time")
...
class TestFindInfotableUrl:
    ...
    @patch("stockdownloader.data.sec.ownership_parser.time")
...
class TestEftsTextFallback:
    ...
    def test_text_fallback_when_xml_empty(
        ...
        with patch("stockdownloader.data.sec.common.fetch_efts_page", return_value=fake_hits), \
             patch("stockdownloader.data.sec.common.fetch_url_text", return_value=text_content):
...
class TestFetchOwnershipSnapshots:
    ...
    def test_returns_empty_when_no_filings_found(
        ...
        with patch.object(client, "_fetch_from_bulk", return_value=None), \
             patch("stockdownloader.data.sec.common.fetch_efts_page", return_value=[]):
...
class TestEftsSnapshotIntegration:
    ...
    def test_cached_quarter_skips_http(self, tmp_path: Path) -> None:
        ...
        with patch(
            "stockdownloader.data.sec.common.fetch_efts_page",
            side_effect=AssertionError("Should not be called"),
        ):
            result = client._fetch_from_efts(
